...
void init(const Variant* v) {
  ...
  for (PieceType pt = PAWN; pt <= KING; ++pt)
  {
      ...
      for (Square s = SQ_A1; s <= SQ_MAX; ++s)
      {
          ...
          Rank r = rank_of(s);
          Rank re = std::max(std::min(r, Rank(v->maxRank - r)), RANK_1);
          psq[ pc][ s] = score + (  pt == PAWN  ? PBonus[std::min(r, RANK_8)][std::min(file_of(s), FILE_H)]
                                  ...
                                  : isSlider    ? make_score(5, 5) * (2 * f + re - v->maxFile - 1)
                                  : isPawn      ? make_score(5, 5) * (2 * f - v->maxFile)
                                                : make_score(10, 10) * (1 + isSlowLeaper) * (f + re - v->maxFile / 2 - (f == 0) - (r == v->maxRank)));
